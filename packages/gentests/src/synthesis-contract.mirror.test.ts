// The synthesis-contract mirror must stay byte-identical to the tech-lead
// declaration (canonical owner @clapp/plan, CLAPP-030 — built in the
...
const FROZEN_CONTRACT = `// ================= SHARED CONTRACT: synthesis-contract.ts =================
...
const mirrorPath = join(here, 'synthesis-contract.ts');

describe('synthesis-contract mirror (canonical owner @clapp/plan, CLAPP-030)', () => {
  test('is byte-identical to the frozen declaration', async () => {
    const onDisk = await readFile(mirrorPath, 'utf8');
    expect(onDisk).toBe(FROZEN_CONTRACT);
  });

  test('declares PLAN_VERSION 0.1 and shapes plan literals structurally', () => {
    expect(PLAN_VERSION).toBe('0.1');
    // Type-level smoke test: the mirror's SynthesisPlan shapes a minimal
    // literal without importing anything from @clapp/plan.
    const skeleton: Pick<
      SynthesisPlan,
      'planVersion' | 'routes' | 'pages' | 'storage' | 'acceptance'
    > = {
      planVersion: '0.1',
      routes: [],
      pages: [],
    ...
    expect(skeleton.routes).toHaveLength(0);
  });
});
