/**
 * @clapp/codegen tests — spawned-app harness (CLAPP-031).
 *
 * Spawns a generated app's server (`bun server.ts`) as a child process
 * with PORT=0 (ephemeral) by default, reads the one JSON line
 * { url, port } the server prints once listening, then polls the health
 * path until the server answers. close() SIGTERMs (SIGKILL fallback).
 */
...
export async function spawnApp(root: string, options: { port?: number } = {}): Promise<SpawnedApp> {
  ...
  if (info === null) {
    child.kill('SIGKILL');
    throw new Error(`generated app at ${root} never reported listening${exited ? ' (process exited)' : ''}; stderr: ${stderr || '(empty)'}`);
  }
