// The diff-contract MIRROR must stay byte-identical to the tech-lead
// declaration (canonical owner @clapp/diff, CLAPP-040 — built in the
// parallel wave). This test pins the mirror against the frozen v0.1
// declaration text verbatim so accidental drift fails loudly; the tech
// lead additionally byte-diffs it against the canonical copy at
// integration.
...
import { DIFF_VERSION } from '../src/diff-contract';
import type { DiffReport, RepairAttempt, RepairDirective, RepairLoopResult } from '../src/diff-contract';

const FROZEN_CONTRACT = `// ============ SHARED CONTRACT: diff-contract.ts ============
...
const mirrorPath = join(here, '../src/diff-contract.ts');

describe('diff-contract mirror (canonical owner @clapp/diff, CLAPP-040)', () => {
  test('is byte-identical to the frozen v0.1 declaration', async () => {
    const onDisk = await readFile(mirrorPath, 'utf8');
    expect(onDisk).toBe(FROZEN_CONTRACT);
  });

  test('declares DIFF_VERSION 0.1 and is fully re-exported from the index', async () => {
    expect(DIFF_VERSION).toBe('0.1');
    const index = await readFile(join(here, '../src/index.ts'), 'utf8');
    expect(index).toContain("export * from './diff-contract'");
    // Type-level smoke: the mirror's own shapes typecheck against the
    // contract surfaces the loop consumes/produces.
    const report: Pick<DiffReport, 'diffVersion' | 'findings' | 'counts'> = {
      diffVersion: DIFF_VERSION,
      findings: [],
      counts: { critical: 0, major: 0, minor: 0, info: 0 },
    };
    const attempt: Pick<RepairAttempt, 'iteration' | 'rerunVerdict'> = {
      iteration: 1,
      rerunVerdict: 'divergent',
    };
    const directive: Pick<RepairDirective, 'id' | 'scopePaths'> = { id: 'repd_x', scopePaths: [] };
    const result: Pick<RepairLoopResult, 'converged'> = { converged: true };
    expect(report.findings).toHaveLength(0);
    expect(attempt.rerunVerdict).toBe('divergent');
    expect(directive.id).toBe('repd_x');
